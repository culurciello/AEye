...
async function showLatestCameraSnapshot() {
    try {
        const response = await fetch('/api/latest_camera_image');
        if (!response.ok) {
        ...
        const data = await response.json();
        if (data.image_url) {
            // Open camera image in a new modal or window
            const modal = document.getElementById('eventModal');
            const title = document.getElementById('modalTitle');
            const body = document.getElementById('modalBody');

            title.textContent = `Camera Feed - ${data.timestamp}`;
            body.innerHTML = `
        ...
        alert('Error loading camera feed');
    }
}