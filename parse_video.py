...
class VideoParser:
    ...
    def process_input(self, input_source: str, confidence_threshold: float = 0.5, 
        ...
        try:
            ...
            for obj_type, obj_detections in detections_by_type.items():
                ...
                top_detections.extend(obj_detections[:top_n])
            
            # Save only the top N detections per object type to database
            print(f"Saving top {len(top_detections)} detections out of {len(detections)} total...")
            saved_detections = [] if not is_stream else None
            for detection_data in top_detections:
                detection_id = self.db.save_detection(**detection_data)
                detection_data['id'] = detection_id
                if saved_detections is not None:
                    saved_detections.append(detection_data)
            ...
            # Create tracks from all detections
    ...
    def process_video(self, video_path: str, confidence_threshold: float = 0.5, top_n: int = 10):
        ...
        self.process_input(video_path, confidence_threshold, top_n, is_stream=False)
...
def main():
    ...
    parser.add_argument('--stream', action='store_true',
                       help='Process as stream instead of video file')
    ...
    if args.stream:
        ...
        parser_obj.process_video(args.input, confidence_threshold=args.confidence, top_n=args.top_n)
